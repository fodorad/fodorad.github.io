...
for (let i = 0; i < l; i++) {
  images[i].addEventListener("click", function () {
    ...
    const imgItem = document.querySelector(".gg-image"), prevBtn = document.querySelector(".gg-prev"), nextBtn = document.querySelector(".gg-next"), closeBtn = document.querySelector(".gg-close");
    ...
    function prev() {
      var index = Array.from(images).indexOf(currentImg);
    
      if (index > 0) {
        prevImg = images[index - 1];
      } else {
        prevImg = images[l - 1]; // Wrap around to the last image
      }
    ...
    function next() {
      ...
      if (index < l - 1) {
        ...
        nextImg = images[0]; // Wrap around to the first image
      }
